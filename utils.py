...
def get_samples(size=1000, type_data="train", deterministic=True, seed="30"):
    
    #lenses_path_train    = os.path.join("data", "train_less_than_2",type_data, "lenses")
    #sources_path_train   = os.path.join("data", "train_less_than_2",type_data, "sources")
    #negatives_path_train = os.path.join("data", "train_less_than_2",type_data, "negatives")
    ...
    #lenses_path_train    = os.path.join("special_data_feb17", type_data, "lenses")
    #sources_path_train   = os.path.join("special_data_feb17",type_data, "sources")
    #negatives_path_train = os.path.join("special_data_feb17",type_data, "negatives")
    

    #sources_fnames, lenses_fnames, negatives_fnames = get_samples(size=sample_size, type_data="train", deterministic=False)
    #sources_fnames=get_sources_samples(size=1000, typ
...
def show3Imgs(img1_numpy, img2_numpy, img3_numpy, img1_title, img2_title, img3_title,folder_name,save_name, invert=True):
    if len(img1_numpy.shape) != 2:
        img1_numpy = np.squeeze(img1_numpy)
    if len(img2_numpy.shape) != 2:
        img2_numpy = np.squeeze(img2_numpy)
    if len(img3_numpy.shape) != 2:
        img3_numpy = np.squeeze(img3_numpy)

    show3Imgs_plot(img1_numpy, img2_numpy, img3_numpy, img1_title, img2_title, img3_title,folder_name+save_name)
    
    if invert:
        #save_name="inv_"+save_name
        show3Imgs_plot(img1_numpy, img2_numpy, img3_numpy, img1_title, img2_title, img3_title,folder_name+"_inv_"+save_name,cmap_name='Greys')
        
    #plt.show()
def show3Imgs_plot(img1_numpy, img2_numpy, img3_numpy, img1_title, img2_title, img3_title, save_name, cmap_name='Greys_r'):
    ...
    plt.grid(b=None)
    plt.axis("off")
    plt.imshow(img1_numpy,cmap=cmap_name)
    plt.title(img1_title)
    
    plt.subplot(1, 3, 2)
    plt.grid(b=None)
    plt.axis("off")
    plt.imshow(img2_numpy, cmap=cmap_name) 
    plt.title(img2_title)
    
    plt.subplot(1, 3, 3)
    plt.grid(b=None)
    plt.axis("off")
    plt.imshow(img3_numpy, cmap=cmap_name) 
